...
u_RMS_i=zeros(te_snaps,max_kept);
...
for n_kept=1:max_kept
    ...
    u_RMS_i(:,n_kept)=sqrt(squeeze(mean(mean((X(:,unknown,te_picks)-g(:,unknown,te_picks)).^2))));
    disp(u_RMS(n_kept))
    % Plot
    if n_kept==1 || k_RMS(n_kept)<best_RMS
        ...
        for i=1:te_snaps;
            subplot(samp_dim,samp_dim,i)
            imshow(cat(2,X(:,unknown,te_picks(i))+X_mean(:,unknown,te_picks(i)),g(:,unknown,te_picks(i))+X_mean(:,unknown,te_picks(i))));            axis equal tight
        end
    end
    figure(4)
